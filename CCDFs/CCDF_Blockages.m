...
H_A1_blockageA1 = H_A1.*(1-0.105);
H_A1_blockageB1 = H_A2.*(1-0.130);
H_A1_blockageC1 = H_A3.*(1-0);
H_A1_blockageD1 = H_A4.*(1- 0.185);


H_A1_blockageA2 = H_A1.*(1-0.105-0.139);
H_A1_blockageB2 = H_A2.*(1-0.130-0);
H_A1_blockageC2 = H_A3.*(1-0-0.133);
H_A1_blockageD2 = H_A4.*(1- 0.185 - 0.063);

H_A1_blockageA3 = H_A1.*(1-0.105-0.139-0);
H_A1_blockageB3 = H_A2.*(1-0.130-0-0.073);
H_A1_blockageC3 = H_A3.*(1-0-0.133-0);
H_A1_blockageD3 = H_A4.*(1- 0.185 - 0.063 - 0.097);

H_A1_blockageA4 = H_A1.*(1-0.105-0.139-0.140);
H_A1_blockageB4 = H_A2.*(1-0.130-0-0.013);
H_A1_blockageC4 = H_A3.*(1-0-0.133-0.033);
H_A1_blockageD4 = H_A4.*(1-0.185-0.063-0);


H_A1_blockageA5 = H_A1.*(1-0.105-0.139-0.140-0);
H_A1_blockageB5 = H_A2.*(1-0.130-0-0.013-0.184);
H_A1_blockageC5 = H_A3.*(1-0-0.133-0.033-0);
H_A1_blockageD5 = H_A4.*(1-0.185-0.063-0-0.129);


P_rec_A1 = P_total.*H_A1_blockageA1.*Ts.*G_Con;
P_rec_B1 = P_total.*H_A1_blockageB1.*Ts.*G_Con;
P_rec_C1 = P_total.*H_A1_blockageC1.*Ts.*G_Con;
P_rec_D1 = P_total.*H_A1_blockageD1.*Ts.*G_Con;

P_rec_A1(abs(receiver_angle)>FOV)=0;
P_rec_B1(abs(receiver_angle2)>FOV)=0;
P_rec_C1(abs(receiver_angle3)>FOV)=0;
P_rec_D1(abs(receiver_angle4)>FOV)=0;
P_rec_total1 = P_rec_A1 + P_rec_B1 + P_rec_C1 + P_rec_D1;
P_rec_dBm1=10*log10(P_rec_total1);
% Var_power = var(P_rec_dBm1);


P_rec_A2 = P_total.*H_A1_blockageA2.*Ts.*G_Con;
P_rec_B2 = P_total.*H_A1_blockageB2.*Ts.*G_Con;
P_rec_C2 = P_total.*H_A1_blockageC2.*Ts.*G_Con;
P_rec_D2 = P_total.*H_A1_blockageD2.*Ts.*G_Con;

P_rec_A2(abs(receiver_angle)>FOV)=0;
P_rec_B2(abs(receiver_angle2)>FOV)=0;
P_rec_C2(abs(receiver_angle3)>FOV)=0;
P_rec_D2(abs(receiver_angle4)>FOV)=0;
P_rec_total2 = P_rec_A2 + P_rec_B2 + P_rec_C2 + P_rec_D2;
P_rec_dBm2=10*log10(P_rec_total2);

P_rec_A3 = P_total.*H_A1_blockageA3.*Ts.*G_Con;
P_rec_B3 = P_total.*H_A1_blockageB3.*Ts.*G_Con;
P_rec_C3 = P_total.*H_A1_blockageC3.*Ts.*G_Con;
P_rec_D3 = P_total.*H_A1_blockageD3.*Ts.*G_Con;

P_rec_A3(abs(receiver_angle)>FOV)=0;
P_rec_B3(abs(receiver_angle2)>FOV)=0;
P_rec_C3(abs(receiver_angle3)>FOV)=0;
P_rec_D3(abs(receiver_angle4)>FOV)=0;
P_rec_total3 = P_rec_A3 + P_rec_B3 + P_rec_C3 + P_rec_D3;
P_rec_dBm3=10*log10(P_rec_total3);

P_rec_A4 = P_total.*H_A1_blockageA4.*Ts.*G_Con;
P_rec_B4 = P_total.*H_A1_blockageB4.*Ts.*G_Con;
P_rec_C4 = P_total.*H_A1_blockageC4.*Ts.*G_Con;
P_rec_D4 = P_total.*H_A1_blockageD4.*Ts.*G_Con;

P_rec_A4(abs(receiver_angle)>FOV)=0;
P_rec_B4(abs(receiver_angle2)>FOV)=0;
P_rec_C4(abs(receiver_angle3)>FOV)=0;
P_rec_D4(abs(receiver_angle4)>FOV)=0;
P_rec_total4 = P_rec_A4 + P_rec_B4 + P_rec_C4 + P_rec_D4;
P_rec_dBm4=10*log10(P_rec_total4);


P_rec_A5 = P_total.*H_A1_blockageA5.*Ts.*G_Con;
P_rec_B5 = P_total.*H_A1_blockageB5.*Ts.*G_Con;
P_rec_C5 = P_total.*H_A1_blockageC5.*Ts.*G_Con;
P_rec_D5 = P_total.*H_A1_blockageD5.*Ts.*G_Con;

P_rec_A5(abs(receiver_angle)>FOV)=0;
P_rec_B5(abs(receiver_angle2)>FOV)=0;
P_rec_C5(abs(receiver_angle3)>FOV)=0;
P_rec_D5(abs(receiver_angle4)>FOV)=0;
P_rec_total5 = P_rec_A5 + P_rec_B5 + P_rec_C5 + P_rec_D5;
P_rec_dBm5=10*log10(P_rec_total5);
...
title('Number of blockages = 2, Pause time = 100 s');
colormap('jet')
colorbar;
...
for i = 1:Nx
    for j = 1:Ny
        if(P_rec_dBm1(i,j) >= 0.5)
            outage1(end+1) = P_rec_dBm1(i,j);
        end        
    end
end
[ycdf1, xcdf1] = cdfcalc(outage1);
xccdf1 = xcdf1;
yccdf1 = 1-ycdf1(1:end-1);

outage2 = zeros(1,1);
for i = 1:Nx
    for j = 1:Ny
        if(P_rec_dBm2(i,j) >= 0.25)
            outage2(end+1) = P_rec_dBm2(i,j);
        end        
    end
end
[ycdf2, xcdf2] = cdfcalc(outage2);
xccdf2 = xcdf2;
...
for i = 1:Nx
    for j = 1:Ny
        if(P_rec_dBm3(i,j) >= 0)
            outage3(end+1) = P_rec_dBm3(i,j);
        end        
    end
end
[ycdf3, xcdf3] = cdfcalc(outage3);
xccdf3 = xcdf3;
yccdf3 = 1-ycdf3(1:end-1);

outage4 = zeros(1,1);
for i = 1:Nx
    for j = 1:Ny
        if(P_rec_dBm4(i,j) >= -0.5)
            outage4(end+1) = P_rec_dBm4(i,j);
        end        
    end
end
[ycdf4, xcdf4] = cdfcalc(outage4);
xccdf4 = xcdf4;
yccdf4 = 1-ycdf4(1:end-1);

outage5 = zeros(1,1);
for i = 1:Nx
    for j = 1:Ny
        if(P_rec_dBm5(i,j) >= -0.5)
            outage5(end+1) = P_rec_dBm5(i,j);
        end        
    end
end
[ycdf5, xcdf5] = cdfcalc(outage5);
xccdf5 = xcdf5;
yccdf5 = 1-ycdf5(1:end-1);


figure(3);
plot(xcdf1,yccdf1, 'linewidth', 1.5);
hold on;
plot(xcdf2,yccdf2, 'linewidth', 1.5);
plot(xcdf3,yccdf3, 'linewidth', 1.5);
plot(xcdf4,yccdf4, 'linewidth', 1.5);
plot(xcdf5,yccdf5, 'linewidth', 1.5);
legend('Blockages = 1', 'Blockages = 2','Blockages = 3','Blockages = 4',...
    'Blockages = 5')
title('CCDF of average received power, Pause Time = 10 seconds');

xlabel('Power received (dBm)');
ylabel('CCDF');
